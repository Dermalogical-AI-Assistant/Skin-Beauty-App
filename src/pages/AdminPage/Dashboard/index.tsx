      ...
      <div className="min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <StartCard title={`Current Sales`} isLoading={isuseFetchMonthlySalesLoading} value={`${getCurrencySymbol("GBP")} ${monthlySales?.newSales||0}`} change={`${(monthlySales?.incrementalRate === 100? monthlySales?.incrementalRate: (monthlySales?.incrementalRate ||0).toFixed(2))}%`} total={`£ ${(monthlySales?.totalSales||0).toFixed(2)}`} changeType={`${monthlySales?.incrementalRate && monthlySales?.incrementalRate>0?"positive":"negative"}`} icon={<TrendingUp />}/>
